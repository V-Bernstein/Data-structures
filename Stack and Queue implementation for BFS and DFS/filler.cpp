...
animation filler::dfs::fillSolid( PNG & img, int x, int y, 
  ...
  return dfs::fill(img, x, y, solid, tolerance, frameFreq);
}
...
animation filler::dfs::fillGrid( PNG & img, int x, int y, 
  ...
  return dfs::fill(img, x, y, grid, tolerance, frameFreq);
}
...
animation filler::dfs::fillGradient( PNG & img, int x, int y, 
  ...
  return dfs::fill(img, x, y, gradient, tolerance, frameFreq);
}
...
animation filler::bfs::fillSolid( PNG & img, int x, int y, 
  ...
  solidColorPicker solid = solidColorPicker(fillColor);
  return bfs::fill(img, x, y, solid, tolerance, frameFreq);
}
...
animation filler::bfs::fillGrid( PNG & img, int x, int y, 
  ...
  return bfs::fill(img, x, y, grid, tolerance, frameFreq);
}
...
return bfs::fill(img, x, y, gradient, tolerance, frameFreq);
}
...
animation filler::fill( PNG & img, int x, int y,
  ...
  animation retAnim; // The animation we're returning
  int width = img.width();
  int height = img.height();
  vector<vector<bool>> processed; // A 2D vector to hold whether a pixel has been visited before
  processed.resize(width, vector<bool>(height));
  int i = x;
  int j = y;
  int frameCount = 0; // Counter to tell us whether we need to add the current image to the animation

  OrderingStructure<int> intStructX;
  OrderingStructure<int> intStructY;
  RGBAPixel orig = *img(x,y); // The starting point. Needed for calculating tolerances
  intStructX.add(x);
  intStructY.add(y);
  while(!intStructX.isEmpty() && !intStructY.isEmpty()){
  
    i = intStructX.remove();
    j = intStructY.remove();
    if(!processed[i][j] && (pow(img(i,j)->red - orig.red, 2) + pow(img(i,j)->green - orig.green, 2) + pow(img(i,j)->blue - orig.blue, 2) <= tolerance)){
      processed[i][j] = true;
      *img(i,j) = fillColor(i,j);
      frameCount ++;
    
      if(i < width-1 && j < height && i >= 0 && j >= 0){
	  intStructX.add(i+1);  // +X direction
	  intStructY.add(j);
	}
      if(i < width && j < height-1 && i >= 0 && j >= 0){
	  intStructX.add(i);
	  intStructY.add(j+1); // +Y direction
	}
	...
	intStructY.add(j-1); // -Y direction
      ...
      if(frameCount % frameFreq == 0){ // Checking to see if it's time for
	retAnim.addFrame(img); // adding an image to the animation
      }
    } // end big if
