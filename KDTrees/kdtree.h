...
class KDTree
{
	...
	void quickSelect(int left, int right, int median, int dimension);
	int partition(int first, int last, int median, int dimension);
	void swap(Point<Dim> & first, Point<Dim> & second) const;
	/* Recursive helper for the KDTree ctor*/
	void ctorRecursive(int first, int last, int dimension);
	/* Recursive helper for fNN*/
	Point<Dim> fNNRecursive(const Point<Dim> & query, Point<Dim> & currBest, int dimension, int lower, int upper) const;
/* Function that returns the agregate distance between two points*/
	int distance(const Point<Dim> first, const Point<Dim> second, int dimension) const;
};
