...
class BinaryTree
{
	...
	/* Private helper function that allows us to use mirror recursively*/
	void mirror(Node* subRoot);

	/* Private helper function that flips the position of the left and right node chains*/
	void flip(Node* subRoot);

	/*Helper function to make a vector with the in-order values*/

	void inOrderVector(Node* subRoot, vector<T> & order) const;

	/*printPaths private helper function. Allows for recursion*/

	void printPaths(Node* subRoot, vector<T> & output) const;
};
